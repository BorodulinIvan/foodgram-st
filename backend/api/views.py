...
class UserProfileView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["can_follow"] = (
            (user.is_authenticated and user.id != profile_user.id)
        )
        context["is_following"] = (
            user.is_authenticated and Follow.objects.filter(
                user=user, author=profile_user).exists()
        )
        return context
...
class FavoritesView(APIView):
    ...
    def post(self, request):
        ...
        serializer_data['cart_count'] = ShoppingCart.objects.filter(
            user=user).count()
...
class CreateRecipeView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['cart_count'] = kwargs.get('cart_count',
                                           ShoppingCart.objects.filter(
                                               user=self.request.user).count())
        return context
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            self.queryset = self.queryset.only('id', 'username', 'first_name',
    ...
    def upload_avatar(self, request):
        ...
        if request.method in ["POST", "PATCH", "PUT"]:
            ...
            try:
                user.avatar = Base64ImageField().to_internal_value(request.data["avatar"])
                user.save()
